...
CREATE PROCEDURE spConsultarPermisosPorIdRol
@IdRol INT
AS
BEGIN
	SELECT * FROM Permisos WHERE IdRol=@IdRol
END

ALTER PROCEDURE spGuardarPermisos
@IdRol INT,
...
BEGIN
	...
	BEGIN
		...
		SELECT '2' AS [Id], 'Se ha actualizado correctamente' AS [Nombre];
	END
		ELSE
		BEGIN
			IF NOT EXISTS (SELECT IdRol FROM Permisos WHERE IdRol=@IdRol)
			BEGIN
				INSERT INTO Permisos (IdRol, Opciones) VALUES (@IdRol, @Opciones);
				SELECT '1' AS [Id], 'Se ha guardado correctamente' AS [Nombre];
			END
				ELSE
				BEGIN
					UPDATE Permisos SET Opciones=@Opciones WHERE IdRol=@IdRol;
					SELECT '2' AS [Id], 'Se ha actualizado correctamente' AS [Nombre];
				END
		END
END
